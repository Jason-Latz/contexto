...
test('quality gate shows verified + rare words and hides ineligible words', async () => {
  await loadLanguagePack('es')
  globalThis.window = { location: { href: 'https://example.test/article' } } as any
  const { extractPageCandidates } = await import('../src/content/injector.js')
  // 'world' = hand-verified (high) → shown. 'snack' = rare medium word → shown
  // (this rare/long-tail vocabulary is the product's value). 'would' = a common
  // function word marked ineligible by the QA pass → never shown.
  // (No onboarding level set in tests → no known-word floor.)
  const candidates = extractPageCandidates([
    { nodeValue: 'The world would offer a healthy snack to anyone reading this page.' } as Text,
  ])
  const lemmas = new Set(candidates.map((candidate) => candidate.lemma))
